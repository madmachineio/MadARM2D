...
static arm_2d_helper_pfb_t * pPFB;
...
void display_task(void)
{
    ...
    while(arm_fsm_rt_cpl != arm_2d_helper_pfb_task(
                                pPFB, NULL));
                                //pPFB, (arm_2d_region_list_item_t *)s_tDirtyRegions));

    //! update performance info
    do {

        int32_t nTotalCyclCount = pPFB->Statistics.nTotalCycle;
        int32_t nTotalLCDCycCount = pPFB->Statistics.nRenderingCycle;
...
void arm_2d_helper_perf_counter_start(void)
{
	...
	start_ticks = swifthal_hwcycle_get();
}
...
/*----------------------------------------------------------------------------
  Main function
 *----------------------------------------------------------------------------*/
int arm_2d_benhmart (arm_2d_helper_pfb_t *p)
{
	pPFB = p;
	example_gui_init();
    ...
    ARM_2D_HELPER_PFB_UPDATE_ON_DRAW_HANDLER(   pPFB,
                                                &__pfb_draw_handler);
